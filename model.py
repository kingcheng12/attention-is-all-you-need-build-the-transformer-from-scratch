...
def build_token_to_id_vocab(sentences, specials=('<pad>', '<bos>', '<eos>', '<unk>')):
    ...
    token_to_id = {specials[i]: i for i in range(len_specials)}
...
def build_id_to_token_vocab(token_to_id):
    ...
    id_to_token = {token_to_id[token]:token for token in token_to_id.keys()}
...
def encode_sentence_to_ids(sentence, token_to_id, unk_token='<unk>'):
    ...
    for token in splitted_sentence:
        if token in token_to_id:
            idx.append(token_to_id[token])
        elif token != '':
            idx.append(token_to_id[unk_token])
...
def pad_id_sequence(ids, max_len, pad_id):
    ...
    pad_ids = ids[:max_len] + [pad_id] * (max(max_len-len(ids), 0))
...
def compute_positional_div_term(d_model):
    ...
    div = [1e4 ** (-2*i/d_model) for i in range(len_div)]
    div = torch.tensor(div, dtype=torch.float)
...
def build_position_index_column(max_len):
    ...
    return torch.tensor(list(range(max_len)), dtype=torch.float).unsqueeze(1)
...
def add_positional_encoding_to_embeddings(embedded_batch, positional_encoding):
    ...
    # embedded_batch shape: (B, L, D)
    # positional_encoding shape: (max_len, D)
...
def softmax_attention_weights(masked_scores):
    ...
    safe_scores = masked_scores.masked_fill(fully_masked, 0.0)
    weights = torch.softmax(safe_scores, dim=-1)

    return weights.masked_fill(fully_masked, 0.0)
...
def merge_heads_back_to_model_dim(multi_head_tensor):
    ...
    return multi_head_tensor.transpose(1, 2).contiguous().reshape(batch_size, seq_len, num_heads * d_k)

# Step 26 - apply_linear_projection
def apply_linear_projection(x, weight, bias):
    # TODO: return x @ weight^T + bias (bias may be None) with shape (..., out_features)
    output = torch.matmul(x, weight.transpose(-2, -1))
...
def merge_heads_and_project_output(context, w_o, b_o):
    ...
    return apply_linear_projection(merged_context, w_o, b_o)
...
def encoder_layer_self_attention_sublayer(x, w_q, w_k, w_v, w_o, gamma, beta, num_heads, src_mask):
    ...
    attention_output = assemble_multi_head_attention_forward(query=x,
                                                            key=x,
                                                            value=x,
                                                            w_q=w_q,
                                                            w_k=w_k,
...
def average_loss_over_non_pad_tokens(total_loss, gold_token_ids, pad_id):
    ...
    non_pad_count = (gold_token_ids != pad_id).sum().clamp_min(1)
...
def compute_token_accuracy_ignoring_pad(log_probabilities, gold_token_ids, pad_id):
    ...
    correct = ((predicted_token_ids == gold_token_ids) & non_pad_mask).sum()

    total = non_pad_mask.sum().clamp_min(1)
